...
class Twi_data extends CI_Model {

	
	public function data(){

		$settings = array(
		    'oauth_access_token' => "4556636492-EaBSdrTnzRflBOUCIkb9PkOLtvNVktaePX8lL6Y",
		    'oauth_access_token_secret' => "vLJlA47q0AFksISE5u2a7PvtfLT8dRdGDKLZfucB1PyvG",
		    'consumer_key' => "d3z0wqpOSFN9X04Zy0lj1d473",
		    'consumer_secret' => "qbCqPPUn1VeGT69D2azsL9SDGym2vjJg1FJr0Zhi57BLiUz9Co"
		);
		...
		$requestMethod = "GET";
		$getfield = '?f=tweets&vertical=default&q='.$search_var.'&src=typd';
        ...
        $url2="https://api.twitter.com/1.1/statuses/retweets/886272058686132225.json";
		...
		//echo json_encode($string['statuses']);
        $size=sizeof($string['statuses']);
        $retweet_count=array();
        $keys=array();
        for($i=0;$i<$size;$i++){
            $retweet_count[$i]=$string['statuses'][$i]['retweet_count'];
            $keys[$i]=$i;
        }
        ...
        for($i=0;$i<$size-1;$i++){
            ...
            for($j=0;$j<$size-$i-1;$j++){
                ...
                if($retweet_count[$j]<$retweet_count[$j+1]){

                    $tmp=$retweet_count[$j];
                    $retweet_count[$j]=$retweet_count[$j+1];
                    $retweet_count[$j+1]=$tmp;
        ...
        for($i=0;$i<$size;$i++){
            $tweet_text[$i]=$string['statuses'][$keys[$i]]['text'];
        }

        //echo json_encode($data);
        return array('tweet_text'=>$tweet_text,'retweet_count'=>$retweet_count);
